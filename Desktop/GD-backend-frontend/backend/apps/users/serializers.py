from .models import User
from rest_framework import serializers
from django.contrib.auth.hashers import make_password, check_password
from secrets import token_hex
import datetime

class UserSerializer(serializers.ModelSerializer):
    class Meta:
        model = User
        fields = ('id', 'name', 'email', 'token', 'token_expired')

class UserSignUpSerializer(serializers.ModelSerializer):
        email = serializers.CharField(required=True)
        password = serializers.CharField(write_only=True, required=True)
        token = serializers.CharField(read_only=True)
        token_expires = serializers.DateTimeField(read_only=True)

        class Mate:
            model = User
            fields = ('id', 'name', 'email', 'password', 'token', 'token_expires')

        # overide the create method
        def create(self, validate_date):

            if User.objects.filter(email=validate_date['email']).exists():
                raise serializers.ValidationError({'email':['This email is already taken.']})

            # encrypt the password
            validate_date['password'] = make_password(validate_date['password'])

            # create a token
            validate_date['token'] = token_hex(30)
            validate_date['token_expires'] = datetime.datetime.now() + datetime.timedelta(days=7)

            return super().create(validate_date)

class UserSignInSerializer(serializers.ModelSerializer):
            email = serializers.EmailField()
            password = serializers.CharField(write_only=True)
            name = serializers.CharField(read_only=True)
            token = serializers.CharField(read_only=True)
            token_expires = serializers.DateTimeField(read_only=True)

            class Meta:
                model = User
                field = ('id', 'name', 'email', 'password', 'token', 'token_expires')

            # overide the create method
            def create(self, validated_data):
                user = User.objects.filter(email=validated_data['email'])

                if len(user) > 0 and check_password(validated_data['password'], user[0].password):
                    user[0].token_expires = datetime.datetime.now() + datetime.timedelta(days=7)


                    return user[0]
                else:
                    raise serializers.ValidationError({'error': 'The password or email is incorrect.'})